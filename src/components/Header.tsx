      ...
      <div className="container-luxe flex items-center justify-between h-16 md:h-20">
        ...
        >
          <span className="relative block h-5 w-5" aria-hidden="true">
            <span
              className={`absolute left-0 top-1 h-0.5 w-5 bg-current transition-transform duration-250 ${open ? "translate-y-1.5 rotate-45" : ""}`}
            />
            <span
              className={`absolute left-0 top-2.5 h-0.5 w-5 bg-current transition-opacity duration-200 ${open ? "opacity-0" : "opacity-100"}`}
            />
            <span
              className={`absolute left-0 top-4 h-0.5 w-5 bg-current transition-transform duration-250 ${open ? "-translate-y-1.5 -rotate-45" : ""}`}
            />
          </span>
        </Button>
