...
function AdminProducts() {
  ...
  const allSelected =
    visible.length > 0 &&
    visible.every((p) => (p.id ? selected.includes(p.id) : false));
...
function ProductDialog({
  ...
  async function uploadImage(file: File) {
    ...
    const { data, error: signErr } = await supabase.storage
      .from("product-images")
      .createSignedUrl(path, 3600 * 24 * 365 * 5);
    setUploading(false);
    if (signErr || !data)
      return toast.error(signErr?.message ?? "Could not read image URL");
  ...
  return (
    <div className="admin-shell fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white text-slate-900 border border-slate-200 shadow-2xl">
